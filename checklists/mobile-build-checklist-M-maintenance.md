...
Small repeatable jobs that don't belong to the A–D build tracks.
Conventions for this repo apply throughout (macros live in
`source/bookends/docinfo.ptx`; targets come from `project.ptx` — read
them, don't assume; the coursebook is external and read-only).
