# Track D — GitHub Pages Deployment
...
## Task D1 — Align the deploy config with the repo (do this first)
...
2. Verify the deploy configuration in `project.ptx` covers every HTML
   target worth publishing (homepage, notes student + instructor) and
   that a landing page ties them together. If a needed target is missing
   (e.g., no web notes target), add it, following the existing
   publication-file pattern (`<version include="notes stu"/>` on the
   student target, `"notes key"` on the instructor target) and mark it
   for deploy.
...
## Task D2 — Preview hygiene (standing rule)
...
2. In the PR body, link the Actions run and the pages that changed
   (deep links, e.g. `.../notes-student/ws-slope-fields.html`), so review
   is one tap per page.
