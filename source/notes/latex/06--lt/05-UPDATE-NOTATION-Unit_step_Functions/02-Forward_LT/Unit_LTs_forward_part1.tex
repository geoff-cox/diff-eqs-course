...
\begin{document}
	...
	{\centering {\sc Forward Laplace Transforms with Unit Step Functions}}

	\noindent\rule{7in}{0.4pt}
	
	\medskip

    The idea of using Laplace transforms to solve a DE with a unit step function remains the same.   The mechanics of finding forward and inverse Laplace transforms of unit step functions is a bit intricate.   The bottom rows of the Laplace Table given on your formula sheet gives the following.



    \[\mathscr{L} \{u_c(t)\} = \frac{e^{-cs}}{s} \]
    \[\mathscr{L} \{f(t)\ u_c(t)\} = e^{-cs} \mathscr{L} \{f(t+c)\}  \]
    \[\mathscr{L} \{f(t-c)\ u_c(t)\} = e^{-cs} F(s)  \]

    The first two transforms are useful when a forward Laplace transform is needed.   The last transform is used when an inverse Laplace transform is needed.  It is now time to try some forward Laplace transformations.


    \section*{Using the Laplace Transform Tables}

    \noindent {\bf Example 1:}  Find the Laplace transform of  \[ q(t)=3u_7(t)+t^2u_4(t) \]

    \newpage

    \noindent {\bf Example 2:}  Find \[ \mathscr{L} \{ e^{5t}u_6(t) \} \] \\
    \vfill
